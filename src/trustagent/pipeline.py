...
def _write_results_md(out: dict, res: Path):
    L = ["# Results (generated by `make eval`)\n",
         f"Brand: **{out['brand']}**  ·  golden n = {out['n_golden']}  ·  seed {out['seed']}  ·  "
         f"retriever = {out['retriever_mode']}\n",
         "## Headline table\n",
    ...
    if "judge_cross_vendor" in out:
        ...
        L += ["\n## Cross-vendor judge check\n",
              f"- Judge A ({cv['judge_a_model']}) on agent: **{cv['A_scores_agent']:.2f}**",
              f"- Judge B ({cv['judge_b_model']}) on agent: **{cv['B_scores_agent']:.2f}**",
              f"- A−B on agent: {cv['A_minus_B_on_agent']:+.2f}  vs  A−B on simple: {cv['A_minus_B_on_simple']:+.2f}"]
